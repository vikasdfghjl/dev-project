...
const AppContent: React.FC = () => {
  ...
  const handleSelectArticle = (article: Article) => {
    ...
    if (!article.isRead) {
      dispatch({
        type: "MARK_ARTICLE_AS_READ_OPTIMISTIC",
        payload: article.id,
      });
  ...
  const sourceArticles = useMemo(() => {
    if (state.selectedFeedId === ALL_ARTICLES_VIEW_ID) {
      ...
      state.articlesByFeed[state.selectedFeedId]
    ) {
  ...
  const handleRefreshFeeds = async () => {
    ...
    try {
      ...
      if (feeds && feeds.length > 0) {
        ...
        const articlePromises = feeds.map((feed: Feed) =>
    ...
    <div className="flex flex-col h-screen bg-muted dark:bg-slate-900 text-foreground dark:text-slate-100 antialiased">
        ...
        <ErrorBoundary>
          <Sidebar
            ...
            onAddFolderClick={() =>
              dispatch({
                type: "OPEN_MODAL",
                payload: { modalName: "addFolderOpen" },
              })
            }
            onEditFolder={folder =>
              dispatch({
                type: "OPEN_MODAL",
                payload: { modalName: "renameFolder", data: folder },
              })
            }
            ...
            onMoveFeed={feed =>
              dispatch({
                type: "OPEN_MODAL",
                payload: { modalName: "moveFeed", data: feed },
              })
            }
            onOpenSettingsClick={() =>
        ...
        <main className="flex-1 flex flex-col overflow-y-auto bg-background dark:bg-slate-850">
          ...
          {state.isSettingsViewOpen ? (
            <ErrorBoundary>
              <SettingsPage
                onCloseSettings={() =>
                  dispatch({ type: "CLOSE_SETTINGS_VIEW" })
                }
                feeds={state.feeds}
                folders={state.folders}
                onAddFeedClick={() => {
            ...
            <AsyncErrorBoundary onRetry={handleRefreshFeeds}>
              <MainContentArea
                ...
                onSortChange={option =>
                  dispatch({ type: "SET_ARTICLE_SORT_OPTION", payload: option })
                }
                articleFilterOption={state.articleFilterOption}
                onFilterChange={option =>
                  dispatch({
                    type: "SET_ARTICLE_FILTER_OPTION",
                    payload: option,
                  })
                }
                articleViewStyle={state.articleViewStyle}
                onArticleViewStyleChange={style =>
                  dispatch({ type: "SET_ARTICLE_VIEW_STYLE", payload: style })
                }
              />
            </AsyncErrorBoundary>
          )}
      ...
      {state.modalState.addFeedOpen && (
        <AddFeedModal
          folders={state.folders}
          onClose={() =>
            dispatch({ type: "CLOSE_MODAL", payload: "addFeedOpen" })
          }
      ...
      {state.modalState.addFolderOpen && (
        <AddFolderModal
          onClose={() =>
            dispatch({ type: "CLOSE_MODAL", payload: "addFolderOpen" })
          }
      ...
      {state.modalState.renameFolder && (
        <RenameFolderModal
          ...
          onClose={() =>
            dispatch({ type: "CLOSE_MODAL", payload: "renameFolder" })
          }
