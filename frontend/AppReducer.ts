...
export const appReducer = (state: AppState, action: AppAction): AppState => {
  switch (action.type) {
    ...
    case 'LOAD_ALL_ARTICLES_SUCCESS':
      return { ...state, isLoadingAllArticles: false, allArticles: action.payload };
    case 'LOAD_ALL_ARTICLES_FAILURE':
      return { ...state, isLoadingAllArticles: false, error: action.payload, allArticles: [] };
    ...
    case 'LOAD_SPECIFIC_ARTICLES_FAILURE':
      return {
        ...
        articlesByFeed: { ...state.articlesByFeed, [action.payload.feedId]: [] }
      };
    case 'SELECT_FEED':
      return {
        ...state,
    ...
    case 'MARK_ARTICLE_AS_READ_OPTIMISTIC': {
      ...
      return {
        ...state,
        allArticles: state.allArticles.map(a => a.id === articleId ? markAsRead(a) : a),
        articlesByFeed: Object.entries(state.articlesByFeed).reduce((acc, [feedId, articles]) => {
          acc[feedId] = articles.map(a => a.id === articleId ? markAsRead(a) : a);
          return acc;
        }, {} as AppState['articlesByFeed']),
        ...(state.selectedArticle?.id === articleId && state.selectedArticle ? { selectedArticle: markAsRead(state.selectedArticle) } : {})
      };
    }
    ...
    case 'DELETE_FEED_SUCCESS': {
      ...
      const newAllArticles = state.allArticles.filter(a => a.feedId !== feedIdToRemove);
      ...
      } else if (state.selectedFeedId === ALL_ARTICLES_VIEW_ID && newFeeds.length === 0){
      ...
      return {
        ...
        selectedArticle: newSelectedFeedId === null || newSelectedFeedId !== state.selectedFeedId ? null : state.selectedArticle,
      };
    }
    ...
    case 'DELETE_FOLDER_SUCCESS':
      return {
        ...
        feeds: state.feeds.map(feed => feed.folderId === action.payload ? { ...feed, folderId: null } : feed),
      };
    ...
    case 'OPEN_MODAL': {
      ...
      if (modalName === 'renameFolder' || modalName === 'moveFeed') {
        return { ...state, modalState: { ...initialState.modalState, [modalName]: data } };
      }
      return { ...state, modalState: { ...initialState.modalState, [modalName]: true } };
    }
    case 'CLOSE_MODAL': {
        const modalName = action.payload;
         if (modalName === 'renameFolder' || modalName === 'moveFeed') {
            return { ...state, modalState: { ...state.modalState, [modalName]: null } };
        }
        return { ...state, modalState: { ...state.modalState, [modalName]: false } };
    }
    ...
    case 'TOGGLE_SETTINGS_VIEW':
      return { 
        ...
        selectedArticle: !state.isSettingsViewOpen ? null : state.selectedArticle, // Clear article if opening settings
      };
