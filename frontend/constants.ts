...
const MOCK_FEEDS_DATA: Feed[] = [
  {
    ...
    favicon: 'https://techcrunch.com/wp-content/uploads/2015/02/tc_favicon.ico?w=32',
  ...
  },
  {
    ...
    favicon: 'https://cdn.vox-cdn.com/uploads/chorus_asset/file/7395351/ operés_favicon.0.ico',
    lastFetched: new Date().toISOString(),
    folderId: 'folder-1',
  },
  {
  ...
  'feed-1': [
    {
      ...
      contentSnippet: 'OmniCorp today announced the release of its groundbreaking AI model, "Prometheus," set to redefine natural language processing capabilities...',
      content: `
        <div class="prose dark:prose-invert max-w-none">
          ...
          <p class="text-gray-500 dark:text-gray-400">Published on ${new Date(Date.now() - 1 * 60 * 60 * 1000).toLocaleDateString()}</p>
    ...
    },
    {
      ...
      contentSnippet: 'QuantumLeap, a startup specializing in quantum algorithm development, has successfully closed a $50 million Series B funding round led by Future Ventures...',
      content: `
        <div class="prose dark:prose-invert max-w-none">
          ...
          <p class="text-gray-500 dark:text-gray-400">Published on ${new Date(Date.now() - 5 * 60 * 60 * 1000).toLocaleDateString()}</p>
  ...
  'feed-2': [
    {
      ...
      contentSnippet: 'Foldable phones have moved from novelty to a mature category. We explore the latest innovations and what the next generation might bring, from rollable screens to new form factors...',
      content: `
        <div class="prose dark:prose-invert max-w-none">
  ...
  'feed-3': [
     {
      ...
      contentSnippet: 'A deep dive into CSS Grid and Flexbox, exploring their strengths, weaknesses, and when to use each for creating responsive and complex web layouts. Includes practical examples and best practices...',
      content: `
        <div class="prose dark:prose-invert max-w-none">
    ...
    },
    {
      ...
      contentSnippet: 'Exploring the principles of ethical design and how to apply them in your UI/UX projects. This guide covers user privacy, dark patterns, accessibility, and creating inclusive digital experiences...',
      content: `
        <div class="prose dark:prose-invert max-w-none">
  ...
  'feed-4': [
    {
      ...
      contentSnippet: 'I built this cool thing, check it out! It does X, Y, and Z. Looking for feedback.',
      content: '<p>I built this cool thing, check it out! It does X, Y, and Z. Looking for feedback.</p>',
      isRead: false,
    }
  ]
};
...
export const getMockArticlesForFeed = (feedId: string): Promise<Article[]> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (articles) {
        resolve(JSON.parse(JSON.stringify(articles.map(a => ({...a, feedId, feedTitle: MOCK_FEEDS_DATA.find(f=>f.id === feedId)?.title }) )))); // Deep copy and ensure feedTitle
      } else {
...
export const addMockFeed = (url: string, folderId?: string | null): Promise<Feed> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const newFeed: Feed = {
        ...
        favicon: `https://www.google.com/s2/favicons?domain=${url.split('/')[2]}&sz=32`,
        lastFetched: new Date().toISOString(),
        folderId: folderId || null,
      };
      MOCK_FEEDS_DATA.push(newFeed);
      MOCK_ARTICLES_DATA[newFeedId] = [ // Add some placeholder articles
        {
          id: `article-${Date.now()}`,
          feedId: newFeedId,
          ...
          contentSnippet: 'This is a sample article automatically generated for the new feed.',
          content: '<p>This is a sample article automatically generated for the new feed.</p>',
          isRead: false,
        },
         {
          ...
          content: '<p>Another sample article to demonstrate content. <strong>Important stuff here.</strong></p>',
          isRead: false,
        }
      ];
...
export const updateMockFolder = (folderId: string, newName: string): Promise<Folder> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
...
export const moveMockFeedToFolder = (feedId: string, folderId: string | null): Promise<Feed> => {
   return new Promise((resolve, reject) => {
    setTimeout(() => {
      const feedIndex = MOCK_FEEDS_DATA.findIndex(f => f.id === feedId);
      if (feedIndex > -1) {
        if (folderId && !MOCK_FOLDERS_DATA.find(folder => folder.id === folderId)) {
