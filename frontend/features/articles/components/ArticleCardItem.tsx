...
const ArticleCardItemComponent: React.FC<ArticleCardItemProps> = ({
  article,
  isSelected,
  onSelect,
  showFeedTitle = false,
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3
          ...
          } ${!article.isRead && !isSelected ? "font-bold" : ""} line-clamp-2`}
        >
          {article.title}
        </h3>
        ...
        <div className="text-xs text-muted-foreground dark:text-slate-500 mt-auto flex flex-col gap-1">
          {showFeedTitle && article.feedTitle && (
            <p
              className="font-medium text-primary/80 dark:text-primary-dark/80 truncate mb-0.5"
          ...
          {article.link && (
            <a
              ...
              className="text-primary hover:underline dark:text-primary-dark text-xs truncate"
            >
              Read full article
            </a>
          )}
