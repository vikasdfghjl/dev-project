...
const ArticleCleanupSectionComponent: React.FC<ArticleCleanupSectionProps> = ({ onCleanup }) => {
  ...
  const handleCleanup = async () => {
    ...
    try {
      ...
      setMessage(`Successfully deleted articles older than ${selectedDays} days`);
  ...
  return (
    <div className="bg-card dark:bg-slate-800 rounded-lg shadow-sm border border-border dark:border-slate-700 p-6">
      ...
      <p className="text-sm text-muted-foreground dark:text-slate-400 mb-4">
        Remove old articles to keep your database clean and improve performance. This action cannot be undone.
      </p>

      <div className="space-y-4">
        <div>
          <label htmlFor="cleanup-days" className="block text-sm font-medium text-muted-foreground dark:text-slate-300 mb-2">
