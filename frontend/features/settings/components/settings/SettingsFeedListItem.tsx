...
const SettingsFeedListItemComponent: React.FC<SettingsFeedListItemProps> = ({
  feed,
  onDelete,
  onMove,
}) => {
    ...
    <li className="flex items-center justify-between p-3 bg-muted/50 dark:bg-slate-700/30 rounded-md hover:bg-muted dark:hover:bg-slate-700/60 transition-colors">
      <div className="flex items-center min-w-0 mr-4">
        ...
        <div className="min-w-0">
          ...
          <p
            className="text-xs text-muted-foreground dark:text-slate-400 truncate"
            title={feed.url}
          >
            {feed.url}
          </p>
      ...
      <div className="flex items-center space-x-2 flex-shrink-0">
        ...
        >
          ...
          {/* Placeholder, consider a folder-move icon */}
        </button>
        <button
          onClick={onDelete}
