  ...
  return (
    ...
    >
      <div className="flex items-center text-foreground dark:text-slate-200 min-w-0">
        {isOpen ? <ChevronDownIcon className="h-5 w-5 mr-2 flex-shrink-0 text-muted-foreground dark:text-slate-400" /> : <ChevronRightIcon className="h-5 w-5 mr-2 flex-shrink-0 text-muted-foreground dark:text-slate-400" />}
        ...
        <span className="ml-2 text-xs text-muted-foreground dark:text-slate-400">({feedCount})</span>
      </div>
      {childrenActions && (
