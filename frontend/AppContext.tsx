...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const [refreshIntervalMinutes] = useLocalStorage(
      ...
      Promise.all([rssService.getFeeds(), rssService.getFolders()])
        ...
        .catch(err =>
          dispatch({
            ...
            payload: err.message || "Failed to refresh feeds",
          })
        );
    };
  ...
  useEffect(() => {
    ...
    Promise.all([rssService.getFeeds(), rssService.getFolders()])
      .then(([feeds, folders]) => {
        ...
        if (
          initialState.selectedFeedId === ALL_ARTICLES_VIEW_ID &&
          feeds.length > 0
        ) {
          dispatch({ type: "LOAD_ALL_ARTICLES_START" });
          const articlePromises = feeds.map(feed =>
            rssService.getArticles(feed.id)
          );
          Promise.allSettled(articlePromises)
            .then(results => {
              ...
              dispatch({
                ...
                payload: successfullyFetchedArticles,
              });
            })
            .catch(err =>
              dispatch({
                type: "LOAD_ALL_ARTICLES_FAILURE",
                payload: err.message || "Failed to load all articles initially",
              })
            );
        }
      })
      .catch(err => {
        ...
        dispatch({
          type: "INIT_APP_FAILURE",
          payload: err.message || "Failed to initialize app",
        });
      });
  }, []);
  ...
  useEffect(() => {
    if (
      ...
      // !state.isLoadingFeeds ensures feeds are loaded
      dispatch({ type: "LOAD_ALL_ARTICLES_START" });
      const articlePromises = state.feeds.map(feed =>
        rssService.getArticles(feed.id)
      );
      Promise.allSettled(articlePromises)
        .then(results => {
          ...
          results.forEach(result => {
            if (result.status === "fulfilled" && result.value) {
              ...
              console.warn(
                `Failed to load articles for a feed during "All Articles" fetch:`,
                result.reason
              );
            }
          });
        ...
        .catch(err =>
          dispatch({
            type: "LOAD_ALL_ARTICLES_FAILURE",
            payload: err.message || "Failed to load all articles",
          })
        );
    } else if (
      state.selectedFeedId === ALL_ARTICLES_VIEW_ID &&
  ...
  const addFeed = useCallback(
    async (url: string, feedName: string, folderId?: string | null) => {
      ...
      try {
        ...
        dispatch({
          ...
          payload: err.message || "Failed to add feed",
        });
  ...
  const deleteFeed = useCallback(async (feedId: string) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: err.message || "Failed to delete feed",
      });
    }
  }, []);
  ...
  const addFolder = useCallback(async (name: string) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: err.message || "Failed to add folder",
      });
      throw err;
    }
  }, []);
    ...
    async (folderId: string, newName: string) => {
      ...
      try {
        ...
        dispatch({
          ...
          payload: err.message || "Failed to rename folder",
        });
        throw err;
      }
    },
  ...
  const deleteFolder = useCallback(async (folderId: string) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: err.message || "Failed to delete folder",
      });
    }
  }, []);

  const moveFeedToFolder = useCallback(
    async (feedId: string, targetFolderId: string | null) => {
      console.log("🔄 MoveFeedToFolder called:", { feedId, targetFolderId });
      lastApiOperationRef.current = Date.now();
      dispatch({ type: "MOVE_FEED_START" });
      try {
        const updatedFeed = await rssService.moveFeedToFolder(
          feedId,
          targetFolderId
        );
        ...
        dispatch({
          type: "MOVE_FEED_FAILURE",
          payload: err.message || "Failed to move feed",
        });
        throw err;
      }
    },
  ...
  const selectFeed = useCallback(
    async (feedId: string | null) => {
      ...
      if (feedId && feedId !== ALL_ARTICLES_VIEW_ID) {
        ...
        if (
          !state.articlesByFeed[feedId] &&
          !state.isLoadingSpecificFeedArticles
        ) {
          dispatch({ type: "LOAD_SPECIFIC_ARTICLES_START", payload: feedId });
          try {
            const articles = await rssService.getArticles(feedId);
            dispatch({
              type: "LOAD_SPECIFIC_ARTICLES_SUCCESS",
              payload: { feedId, articles },
            });
          } catch (err: any) {
            dispatch({
              type: "LOAD_SPECIFIC_ARTICLES_FAILURE",
              payload: {
                feedId,
