...
@router.post(
    "/", response_model=folder_schemas.Folder, status_code=status.HTTP_201_CREATED
)
def create_folder(
    folder: folder_schemas.FolderCreate, db: Session = Depends(database.get_db)
):
...
def delete_folder(folder_id: str, db: Session = Depends(database.get_db)):
    ...
    raise HTTPException(
        ...
        detail=result.get("error", "Folder not found"),
    )
