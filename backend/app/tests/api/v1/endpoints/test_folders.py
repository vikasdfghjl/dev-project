...
from app.schemas.folder import (  # FolderCreate/Update not directly used in these tests but good for context
    FolderCreate,
    FolderUpdate,
)
...
def _create_folder_via_api(
    client: TestClient, name: str
) -> dict:  # Renamed for clarity (internal helper)
...
class TestFolderEndpoints:
    def test_create_folder_success(self, client: TestClient, db_session: Session):
        ...
        folder_id = data["id"]
        db_folder = (
            db_session.query(FolderModel).filter(FolderModel.id == folder_id).first()
        )
        assert db_folder is not None
        assert db_folder.name == folder_name

    def test_create_folder_duplicate_name(
        self, client: TestClient, db_session: Session
    ):  # db_session might not be strictly needed if not verifying DB directly after API call
    ...
    def test_list_folders_with_content(
        self, client: TestClient, db_session: Session
    ):  # db_session not strictly needed here if relying on API state
    ...
    def test_rename_folder_success(self, client: TestClient, db_session: Session):
        ...
        rename_response = client.put(
            f"/api/v1/folders/{folder_id}", json={"name": new_name}
        )
    ...
    def test_rename_folder_non_existent(self, client: TestClient):
        ...
        response = client.put(
            f"/api/v1/folders/{non_existent_id}", json={"name": "Doesnt Matter"}
        )
        assert response.status_code == 404

    def test_rename_folder_to_duplicate_name(
        self, client: TestClient, folder_factory
    ):  # Use folder_factory if direct DB setup is preferred
        ...
        _ = _create_folder_via_api(
            client, name2
        )  # Create the second folder that name1 will conflict with
    ...
    def test_rename_folder_invalid_data(
        self, client: TestClient, db_session: Session
    ):  # db_session for potential direct verification if needed
        ...
        # Test with empty name
        response_empty_name = client.put(
            f"/api/v1/folders/{folder_id}", json={"name": ""}
        )
        assert response_empty_name.status_code == 422

        # Test with name too long (Folder model: String(100))
        long_name = "b" * 101
        response_long_name = client.put(
            f"/api/v1/folders/{folder_id}", json={"name": long_name}
        )
        assert response_long_name.status_code == 422
