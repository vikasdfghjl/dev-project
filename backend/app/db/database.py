...
def create_database_engine():
    ...
    logger.info(f"Attempting to connect to database: {SQLALCHEMY_DATABASE_URL.split('@')[-1] if '@' in SQLALCHEMY_DATABASE_URL else 'Local database'}")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                logger.warning(f"⚠️ Database connection test failed (attempt {attempt}/{max_retries}): {str(e)}")
...
try:
    ...
    logger.critical("🛑 Database connection failed, creating fallback Base for graceful degradation")
...
def get_db():
    ...
    if SessionLocal is None:
        logger.error("❌ Database session factory not initialized - check database connection")
